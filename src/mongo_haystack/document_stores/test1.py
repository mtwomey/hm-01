...
def mongo_filter_converter(filter):
    # todo
    return {}


def _target_filter_to_metadata(filter, metadata_field_name) -> Union[dict, list]:
    """
    Returns a new filter with any non-operator keys renamed so that the metadata field
    name is prepended. Does not mutate input filter.
...
def _and_or_to_list(filter) -> Union[dict, list]:
    """
